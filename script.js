...
function cartItemClickListener(selectProduct) {
  const selectProduct = async (id) => {
    fetch(`https://api.mercadolibre.com/items/${ItemID}`).then(
        (response) => {
          response.json().then((details) => {
            const productDetails = details;
          details.find((element) => element === id).then(
            (ol.appendChild(createProductItemElement(element))),
          },
        },
      ).catch((erro) => {
